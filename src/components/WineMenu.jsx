        ...
        <div className="grid gap-12 lg:grid-cols-[0.82fr_1.18fr]">
          ...
          >
            ...
            <h2 id="wine-menu-title" className="font-display text-[clamp(2.5rem,4.5vw,4.5rem)] font-extrabold leading-[0.82]">
              Dziś coś lekkiego czy konkretnego?
            </h2>
            ...
            <dl className="mt-6 flex border-t border-ink/10 pt-6" aria-label="Ceny wina z kranu">
              {tapPricing.map(({ size, price }) => (
                <div key={size} className="flex flex-1 flex-col items-center gap-1 border-r border-ink/10 last:border-r-0">
                  ...
                  <dd className="font-display text-[1.7rem] font-black leading-none text-wine">{price}</dd>
                </div>
              ))}
            </dl>
          </motion.div>
          ...
          >
            <ul className="grid grid-cols-3 gap-3" aria-label="Aktualne wina z kranu">
              {tapWines.map((wine) => (
                <li key={wine.name}>
                  <article className="wine-card group relative flex min-h-[120px] flex-col items-center justify-center overflow-hidden rounded-[5px] bg-ink px-3 py-8 text-center text-bone" tabIndex={0}>
                    <BottleIcon variant={wine.variant} />
                    <h3 className="wine-card-title m-0 font-display text-[0.92rem] font-black leading-[1.15] text-bone">{wine.name}</h3>
                    <p className="wine-card-type font-display text-[0.68rem] font-bold uppercase tracking-[0.14em] text-bone/40">{wine.type}</p>
                    <p className="wine-card-description absolute inset-4 z-[1] m-0 flex translate-y-2 items-center justify-center text-[0.82rem] font-bold leading-[1.25] text-bone opacity-0">
                      {wine.description}
                    </p>
                  </article>
                </li>
